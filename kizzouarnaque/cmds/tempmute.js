...
exports.run = async(client, message, args) => {
    ...
    let time = args[1];
    if(!time || !ms(time)) return message.reply(`:x: | Vous devez spécifier un temps **valide**.`);
    ...
    message.channel.send(`${mutedUser.user} a été mute avec succès pour ${ms(ms(time), {long:true}).replace("day", "jour").replace("hour", "heure").replace("second", "seconde")}`)
    ...
    setTimeout(function(){
        ...
        message.channel.send(`${mutedUser.user} a bien été démute !`).then(msg=>msg.delete(5000))
